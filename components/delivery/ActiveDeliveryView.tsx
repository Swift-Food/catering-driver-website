  ...
  const handlePhotoSelected = useCallback(
    async (stopId: string, file: File) => {
      ...
      try {
        ...
        const url = await cateringDriverApi.uploadImage(compressedFile);
    ...
    <div className="space-y-6 animate-in fade-in duration-700">
      ...
      <div className="grid lg:grid-cols-5 gap-6 items-start">
        <div className="lg:col-span-3 space-y-6">
          ...
          {viewMode === "TIMELINE" ? (
            <div className="bg-surface rounded-2xl relative">
              {uploadError && (
...
function deriveStops(
  ...
  const pickupStops: DeliveryStop[] = session.restaurants
    ...
    .map((restaurant) => {
      ...
      // Derive checklist items from restaurant menuItems
      const items: ChecklistItem[] | undefined = restaurant.menuItems?.map((mi) => ({
        id: mi.menuItemId,
        label: mi.name,
